...
- Backported event routes from the development line. `ducknng_add_event_route(service_name, path, handler_sql[, heartbeat_ms])` registers a `GET` route that relays one NNG PUB/SUB subscription to an HTTP client as Server-Sent Events. The handler SQL names the subscription once per request; a relay thread owned by the HTTP server then dials, subscribes, and writes `: ready`, one event per message, and `: keepalive` comments, holding no DuckDB connection or request slot. Relays receive in 200 ms slices, so stopping the server only marks it stopping, cancels blocked writes, and joins the relays; it never closes a relay socket from another thread. `ducknng_list_http_routes()` gains `is_event_route` and `event_heartbeat_ms`. Covered by `test/sql/ducknng_event_routes.test`, `make event_route_smoke` (Python `http.client`), and `make event_route_race`.
- Backported `ducknng_encode_rpc_call(method, payload)` from the development line: it builds the call frame for one manifest-declared JSON method, the encoding counterpart of `ducknng_decode_frame()`, so SQL clients no longer assemble header bytes by hand.
